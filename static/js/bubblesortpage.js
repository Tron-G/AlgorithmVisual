...
function clearAllTimer(animation_data, do_clear) {
    if (do_clear === true) {
        ...
        animation_data.explain_words = ["数组创建完成", "请点击开始按钮运行算法", "遍历数组",
            "当前数字大于后一位数字，交换两个数字", "排序完成"];
    }
}
...
function drawSample(svg_data) {
    svg_data.m_svg.append("g")
        .attr("class", "g_sample");
    let sample_rect = [svg_data.rect_fill, svg_data.rect_search_fill, svg_data.rect_choose_fill, svg_data.rect_done_fill];
    let sample_text = ["未排序元素", "查找中的元素", "交换元素", "已归位元素"];
...
function drawConclusion(svg_data, post_data) {
    ...
    for (let idx = 0; idx < post_data.sort_process.length; idx++) {
        sum += post_data.sort_process[idx].length;
    }
...
function sortAnimation(svg_data, post_data, animation_data) {
    ...
    for (let idx = 0; idx < post_data.sort_process.length; idx++) {
        if (post_data.sort_process[idx].length === 0)
            continue;
        let step = 0;
        for (let j = 0; j < (post_data.array_data.length - 1); j++) {        //每一趟排序
            temp_frame = function () {                                  //查找
            ...
            if (j === post_data.sort_process[idx][step][1]) {                //交换
                temp_frame = function () {
                    drawCode(post_data, animation_data, 3, 1);
                    svg_data.m_svg.selectAll(".m_rect" + j)
                        .transition()
                        .duration(animation_data.duration / 2)
                        .attr("fill", svg_data.rect_choose_fill)
                        .attr("x", svg_data.rect_x[j + 1]);
                    svg_data.m_svg.selectAll(".m_rect" + (j + 1))
                        .transition()
                        .duration(animation_data.duration / 2)
                        .attr("fill", svg_data.rect_choose_fill)
                        .attr("x", svg_data.rect_x[j]);

                    svg_data.m_svg.selectAll(".m_rect" + j).attr("class", "temp_class");
                    svg_data.m_svg.select("#m_rect" + j).attr("id", "temp_rect");
                    svg_data.m_svg.select("#rect_text" + j).attr("id", "temp_text");

                    svg_data.m_svg.selectAll(".m_rect" + (j + 1)).attr("class", "m_rect" + j);
                    svg_data.m_svg.select("#m_rect" + (j + 1)).attr("id", "m_rect" + j);
                    svg_data.m_svg.select("#rect_text" + (j + 1)).attr("id", "rect_text" + j);

                    svg_data.m_svg.selectAll(".temp_class").attr("class", "m_rect" + (j + 1));
                    svg_data.m_svg.select("#temp_rect").attr("id", "m_rect" + (j + 1));
                    svg_data.m_svg.select("#temp_text").attr("id", "rect_text" + (j + 1));
                };
                animation_data.sortframe.push(temp_frame);

                step += 1;
                if (step > (post_data.sort_process[idx].length - 1)) {
                    temp_frame = function () {
                        svg_data.m_svg.selectAll("#m_rect" + post_data.sort_process[idx][post_data.sort_process[idx].length - 1][1])
                            .transition()
                            .duration(animation_data.duration / 2)
                            .attr("fill", svg_data.rect_fill);
                        svg_data.m_svg.selectAll("#rect_text" + post_data.sort_process[idx][post_data.sort_process[idx].length - 1][1])
                            .transition()
                            .duration(animation_data.duration / 2)
                            .attr("fill", svg_data.num_fill);
                        for (let k = post_data.sort_process[idx][post_data.sort_process[idx].length - 1][2]; k < post_data.array_data.length; k++) {
                            svg_data.m_svg.selectAll("#m_rect" + k)
                                .transition()
                                .duration(animation_data.duration / 2)
                                .attr("fill", svg_data.rect_done_fill);
                            svg_data.m_svg.selectAll("#rect_text" + k)
                                .transition()
                                .duration(animation_data.duration / 2)
                                .attr("fill", svg_data.rect_done_fill);
                            is_done += 1;
    ...
    if (is_done < post_data.array_data.length) {
        temp_frame = function () {
            for (let p = 0; p < post_data.array_data.length; p++) {
                svg_data.m_svg.selectAll("#m_rect" + p)
                    .transition()
                    .duration(animation_data.duration / 2)
                    .attr("fill", svg_data.rect_done_fill);
                svg_data.m_svg.selectAll("#rect_text" + p)
                    .transition()
                    .duration(animation_data.duration / 2)
                    .attr("fill", svg_data.rect_done_fill);
            }
...
function drawCode(post_data, animation_data, word_id, now_step) {
    ...
    if (post_data.operate_type === 1)
        code_text = ["for i = 0 to a.length - 1/for j = 0 to a.length - i - 1", "if(a[j] > a[j+1])/Exchange(a[j], a[j+1])"];
    else
        code_text = ["请执行排序操作"];
